...
3. Call an available permission-recording tool as soon as the scopes are known and before writing permission evidence. Its `recorded` result means setup was requested, not that the app or active installation grants access; keep the capability blocked.
...
7. Treat `401` and `403` as permission/setup failures in HTTP endpoints and `src/modules` web methods. Log the original server error, inspect `error.status` or `error.response?.status`, and return or throw `{ code: "MISSING_PERMISSION", message: "...", requiredScopes: [...] }`. Use `UNSUPPORTED_CAPABILITY`, `SITE_UNPUBLISHED`, or `TRANSIENT_FAILURE` for those cases; only the transient state may offer Retry. Do not collapse permission failures into a generic error.

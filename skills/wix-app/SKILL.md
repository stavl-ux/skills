...
4. Read only the exact API/component documentation named by the accepted route. Before importing an SDK or host module, identify the execution host (Dashboard, Site, Editor, or backend) and confirm the selected method supports that host; TypeScript compatibility is not runtime evidence. Then implement only in generated files.

...
4. Read only the exact API/component documentation named by the accepted route. Before importing an SDK or host module, identify the execution host (Dashboard, Site, Editor, or backend), confirm the selected method supports that host, and verify every required app scope is granted. `auth.elevate()` changes execution identity; it never grants a missing app scope. TypeScript compatibility is not runtime evidence. Then implement only in generated files.

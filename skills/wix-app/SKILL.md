...
4. Read only the exact API/component documentation named by the accepted route. Before importing an SDK or host module, identify its execution host, confirm the method supports that host, and verify every required app scope is granted. Call an available permission tool as soon as scopes are known; `recorded` requests setup but does not prove the app or installation grants access. `auth.elevate()` changes identity; it never grants a missing scope. TypeScript compatibility is not runtime evidence. Then implement only in generated files.
